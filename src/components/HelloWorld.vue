    ...
    <el-row :gutter="30">
      ...
      >
        ...
        <div class="list-content" v-else>
          ...
          <div
            :class="['textInfo', ClassName[index]]"
            :style="{ fontFamily: fontSeachType[index] }"
            :key="RefreshModel + index"
          >
            {{ fontTextList[index] }}
          </div>
        </div>
    ...
    changeShowType() {
      ...
      if (this.showType) {
        ...
        this.FontList.forEach((item) => {
          ...
          this.fontSeachType.push(`font-poem-${item.id}`);
        });

        document.fonts.ready.then(() => {
          this.RefreshModel = new Date().getTime();
        });
    ...
    submitSearch() {
      ...
      this.$axios({
        ...
        res.data.List.forEach((item) => {
          ...
          let style = document.createElement("style");
          style.innerText = `@font-face{font-family: "${item.FontFamily}";src: url("${url2}.ttf") format("TrueType"), url("${url2}.woff") format("woff"), url("${url2}.eot") format("embedded-opentype");font-display: swap;}.${item.ClassName}{font-family: "${item.FontFamily}"}`;
          document.getElementsByTagName("head")[0].appendChild(style);
